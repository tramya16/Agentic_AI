...
class ResearchFocusedLLMComparator:
    def __init__(self, base_dir: str = "old_results"):
        self.base_dir = Path(base_dir)
        
        # Create only pipeline-specific directories as main directories
        self.single_shot_dir = Path("single_shot")
        self.iterative_dir = Path("iterative")
    ...
    def create_research_visualizations(self, all_llm_evaluations: Dict):
        ...
        print(f"Debug: mol_df Pipeline values: {mol_df['Pipeline'].value_counts() if 'Pipeline' in mol_df.columns else 'Pipeline column missing'}")
            ...
            try:
                ...
                try:
                    ax2.hist([ss_scores, it_scores], bins=20, alpha=0.7, 
                            label=['Single-Shot', 'Iterative'], color=[self.colors['single_shot'], self.colors['iterative']])
                    ax2.legend()
                except:
    ...
    def create_rq2_llm_performance_comparison(self, comp_df: pd.DataFrame, mol_df: pd.DataFrame,
        ...
        for llm_name, llm_eval in all_llm_evaluations.items():
            ...
            coverage = summary["successful_queries"] / summary["total_queries"] * 100 if summary["total_queries"] > 0 else 0
    ...
    def create_molecular_level_analysis(self, molecular_metrics: Dict, viz_dir: Path):
        ...
        if not metrics_df.empty:
            ...
            bars1 = ax2.bar(x_pos - width / 2, [metrics_df[(metrics_df['LLM'] == llm) & 
                                                           (metrics_df['Pipeline'] == 'Single-Shot')]['mean_score'].iloc[0] 
            ...
            bars2 = ax2.bar(x_pos + width / 2, [metrics_df[(metrics_df['LLM'] == llm) & 
                                                           (metrics_df['Pipeline'] == 'Iterative')]['mean_score'].iloc[0] 
                                                if len(metrics_df[(metrics_df['LLM'] == llm) & 
            ...
            for pipeline in ['single_shot', 'iterative']:
                ...
                if molecules:
                    ...
                    metrics = {
                        ...
                        'high_score_rate': len([s for s in valid_scores if s > 0.8]) / len(valid_scores) if valid_scores else 0,
    ...
    def create_comprehensive_tables(self, all_llm_evaluations: Dict, comp_df: pd.DataFrame,
        ...
        for llm_name, llm_evaluation in all_llm_evaluations.items():
            ...
            self._create_pipeline_specific_llm_tables(llm_name, llm_evaluation, "single_shot", self.single_shot_dir / "tables")
            # Create only iterative specific tables in iterative directory
            self._create_pipeline_specific_llm_tables(llm_name, llm_evaluation, "iterative", self.iterative_dir / "tables")
    ...
    def create_pipeline_visualizations(self, pipeline_evaluations: Dict, pipeline_type: str, viz_dir: Path):
        ...
        if not comp_df.empty:
            ...
            self._create_pipeline_llm_performance_analysis(comp_df, mol_df, pipeline_evaluations, pipeline_type, viz_dir)
    ...
    def save_pipeline_data(self, pipeline_evaluations: Dict, pipeline_type: str, data_dir: Path):
        ...
        for llm_name, llm_eval in pipeline_evaluations.items():
            ...
            summary_data.append({
                ...
                'Total_Queries': summary["total_queries"],
                'Successful_Queries': summary["successful_queries"],
                'Coverage_Percent': (summary["successful_queries"] / summary["total_queries"] * 100) if summary["total_queries"] > 0 else 0,
    ...
    def _create_pipeline_validity_novelty_diversity_analysis(self, mol_df: pd.DataFrame, pipeline_type: str, viz_dir: Path):
    ...
    def _write_cross_llm_table(self, all_task_data: Dict, llm_names: List[str], pipeline_name: str, filename: Path):
        ...
        for task_name in sorted_tasks:
            ...
            winners = [llm for llm in llm_names if llm in task_data and task_data[llm]['auc'] == best_auc and best_auc > 0]
            if len(winners) == 1:
...
"""
    ...
    def _create_pipeline_specific_llm_tables(self, llm_name: str, llm_evaluation: Dict, pipeline_type: str, tables_dir: Path):
        ...
        for query_name, query_eval in llm_evaluation["query_evaluations"].items():
            ...
            if pipeline_data and pipeline_data.get("auc_scores"):
                ...
                best_val = max([run["max_score"] for run in pipeline_data["runs"]]) if pipeline_data.get("runs") else 0.0
    ...
    def _generate_pipeline_specific_latex_table(self, llm_name: str, task_data: list, pipeline_type: str, tables_dir: Path):
...
"""
    ...
    def _create_top_k_smiles_overlap_analysis(self, pipeline_evaluations: Dict, pipeline_type: str, viz_dir: Path):
            ...
            for query_name, query_eval in llm_eval["query_evaluations"].items():
                ...
                if pipeline_data["runs"]:
                    ...
                    task_overlap_data[query_name][llm_name] = {
                        ...
                        'top_5': set([mol['SMILES'] for mol in sorted_molecules[:5]]) if len(sorted_molecules) >= 5 else set([mol['SMILES'] for mol in sorted_molecules]),
                        'top_10': set([mol['SMILES'] for mol in sorted_molecules[:10]]) if len(sorted_molecules) >= 10 else set([mol['SMILES'] for mol in sorted_molecules])
                    }
        ...
        if not overlap_df.empty:
            ...
            if not heatmap_data.empty:
                ...
                ax1.set_yticklabels([task[:15] + '...' if len(task) > 15 else task for task in heatmap_data.index], fontsize=9)
        ...
        if len(tasks_to_show) > 0:
            ...
            for task in tasks_to_show:
                ...
                top1_val = task_data[task_data['K_Level'] == 'top_1']['Avg_Jaccard'].iloc[0] if len(task_data[task_data['K_Level'] == 'top_1']) > 0 else 0
                top5_val = task_data[task_data['K_Level'] == 'top_5']['Avg_Jaccard'].iloc[0] if len(task_data[task_data['K_Level'] == 'top_5']) > 0 else 0
                top10_val = task_data[task_data['K_Level'] == 'top_10']['Avg_Jaccard'].iloc[0] if len(task_data[task_data['K_Level'] == 'top_10']) > 0 else 0
    ...
    def _create_comprehensive_cross_llm_smiles_overlap(self, pipeline_evaluations: Dict, pipeline_type: str, viz_dir: Path):
        ...
        self._create_overlap_analysis_part2(llm_all_smiles, task_specific_smiles, llm_names, pipeline_name, pipeline_type, viz_dir)
        self._create_overlap_analysis_part3(llm_all_smiles, pipeline_evaluations, llm_names, pipeline_name, pipeline_type, viz_dir)
        
        print(f"    ✓ Created 3-part cross-LLM SMILES overlap analysis for {pipeline_type}")

    def _create_overlap_analysis_part1(self, llm_all_smiles: Dict, llm_names: List, pipeline_name: str, pipeline_type: str, viz_dir: Path):
        ...
        if len(llm_names) > 1:
            ...
            if non_zero_data:
                ...
                short_labels = [label.replace('Unique to 1 LLM', 'Unique').replace('Shared by 2 LLMs', '2-LLM').replace('Shared by 3+ LLMs', '3+ LLM') for label in labels]
            ...
            if not top_molecules.empty:
                ...
                for llm in mol_df['LLM'].unique():
                    ...
                    if not llm_top.empty:
                        ...
                        normalized_props.append(1 - min(abs(llm_top['LogP'].mean()) / 10, 1))  # Closer to optimal range is better
    ...
    def create_intuitive_performance_overview(self, all_llm_evaluations: Dict, comp_df: pd.DataFrame,
        ...
        if not comp_df.empty:
            ...
            for bars in [bars1, bars2] if 'Single-Shot' in pipeline_comparison.columns and 'Iterative' in pipeline_comparison.columns else []:
                for bar in bars:
                    height = bar.get_height()
                    if height > 0:
    ...
    def create_pipeline_success_breakdown(self, pipeline_evaluations: Dict, pipeline_type: str, viz_dir: Path):
        ...
        if all_molecules:
            ...
            # Filter out zero values
            non_zero_diff = [(count, label, color) for count, label, color in zip(difficulty_counts, difficulty_labels, difficulty_colors) if count > 0]
            if non_zero_diff:
    ...
    def create_llm_comparison_dashboard(self, all_llm_evaluations: Dict, comp_df: pd.DataFrame, viz_dir: Path):
        ...
        if not summary_df.empty:
            ...
            for bar, val in zip(bars, summary_df['Total_Molecules']):
                ax5.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(summary_df['Total_Molecules'])*0.02,
...
def main():
    ...
    comparator = ResearchFocusedLLMComparator(base_dir="old_results")
